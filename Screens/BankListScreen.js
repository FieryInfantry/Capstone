...
const BankListScreen = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`http://192.168.86.249:3000/banks/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSave = async (bankDetails) => {
    try {
      ...
      if (selectedBank) {
        // Update bank
        await axios.put(`http://192.168.86.249:3000/banks/${selectedBank._id}`, bankDetails, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        // Add new bank
        const response = await axios.post('http://192.168.86.249:3000/banks', bankDetails, {
          headers: { Authorization: `Bearer ${token}` },
        });
