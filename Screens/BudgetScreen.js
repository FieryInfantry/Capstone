...
const BudgetScreen = () => {
  ...
  const handleSaveBudget = async (categoryName, amount, currentDate) => {
    ...
    try {
      ...
      const bankBalanceResponse = await fetch(
        "http://localhost:3000/banks/balances",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const totalBudgetResponse = await fetch(
        "http://localhost:3000/budget/total",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const deleteBudget = async (categoryName) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/budget?category=${categoryName}&month=${
          month + 1
        }&year=${year}`,
        {
          method: "DELETE",
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.budgetCard}>
              ...
              <Text style={styles.budgetDetails}>
                Remaining: ₱
                {calculateRemaining(item.limit, item.spent).toFixed(2)}{" "}
              </Text>{" "}
              <View style={styles.progressBar}>
                <View
                  style={{
                    ...
                    backgroundColor:
                      item.spent > item.limit ? "red" : "#4caf50",
                  }}
                />{" "}
              </View>{" "}
              {item.spent > item.limit && (
                <Text style={styles.limitExceeded}> * Limit exceeded </Text>
              )}{" "}
              <TouchableOpacity
  ...
  const renderIncomeExpenseContent = () => (
    <View style={styles.incomeExpenseContainer}>
      <Text style={styles.incomeExpenseText}>
        {" "}
        Income & Expense content will go here!{" "}
      </Text>{" "}
    </View>
  );
