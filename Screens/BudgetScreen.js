...
const BudgetScreen = () => {
  ...
  const fetchIncomes = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/incomes/monthly?month=${month + 1}&year=${year}`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleSaveBudget = async (categoryName, amount, currentDate) => {
    ...
    try {
      ...
      const bankBalanceResponse = await fetch('http://localhost:3000/banks/balances', { // Replace with your IP
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      ...
      const response = await fetch('http://localhost:3000/budget', { // Replace with your IP
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const deleteIncome = async (incomeId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/income/${incomeId}`, // Using _id as the identifier
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const deleteExpense = async (id) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/expense/${id}`,  // Using ID in the URL
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
